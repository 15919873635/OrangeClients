package com.jwis.request.client.window;

import org.eclipse.swt.widgets.Display;
import org.eclipse.swt.widgets.FileDialog;
import org.eclipse.swt.widgets.Shell;

import com.jwis.request.client.constant.WindowConstant;
import com.jwis.request.client.util.ClientUtil;
import com.jwis.request.client.util.HttpClientUtil;
	...
	private void initWinContent(){
			...
			public void widgetSelected(SelectionEvent paramSelectionEvent) {
				...
				if(StringUtils.isNotBlank(urltext) && protocol.equals(WindowConstant.REQUEST_HTTP)){	
					...
					if(method.equals(WindowConstant.REQUEST_GET))
						...
						if(!bodytext.equals("") && bodytext.length() > 0){
							bodyMap.put("body", bodytext);
						}
						if(filePaths != null && filePaths.length() > 0){
							String[] chooseFilePaths = filePaths.split(";");
							if(chooseFilePaths != null && chooseFilePaths.length > 0){
								int fileCount = 0;
		...
		fileChooseButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				if(!chooseFilePaths.equals("") && chooseFilePaths.length() > 0){
